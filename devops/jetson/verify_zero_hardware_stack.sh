...
X0=$(timeout 10 ros2 topic echo /odom --once --field pose.pose.position.x 2>/dev/null | head -1)
X0=${X0:-unset}
echo "   x0 = $X0"
...
VEL=$(timeout 10 ros2 topic echo /odom --once --field twist.twist.linear.x 2>/dev/null | head -1)
VEL=${VEL:-0}
...
TA=$(date +%s.%N); XA=$(timeout 10 ros2 topic echo /odom --once --field pose.pose.position.x 2>/dev/null | head -1)
sleep "$DRIVE_SECONDS"
TB=$(date +%s.%N); XB=$(timeout 10 ros2 topic echo /odom --once --field pose.pose.position.x 2>/dev/null | head -1)
pkill -f "[r]os2 topic pub" 2>/dev/null || true
...
sleep "$WATCHDOG_WAIT"
X1=$(timeout 10 ros2 topic echo /odom --once --field pose.pose.position.x 2>/dev/null | head -1)
X1=${X1:-0}
...
X2=$(timeout 10 ros2 topic echo /odom --once --field pose.pose.position.x 2>/dev/null | head -1)
sleep 1.5
X3=$(timeout 10 ros2 topic echo /odom --once --field pose.pose.position.x 2>/dev/null | head -1)
