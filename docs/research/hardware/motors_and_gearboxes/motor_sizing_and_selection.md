...
## 2. Torque envelope per motor (r = 0.06 m, 4 driven)

| Case | Command | Torque per wheel | Nature |
|---|---|---|---|
| Flat cruise (crr 0.05) | steady state | **0.20 Nm** | continuous (thermal) |
| 5° ramp + 0.5 m/s² accel, SF 2.0 | design case | **1.47 Nm** | rated / short-duty target |
| Emergency stop 0.75→0 in 0.25 m | transient | 1.97 Nm | brief peak |
| **Pivot-in-place on carpet** (μ 0.6–0.8) | worst case | **2.9–3.8 Nm** | peak / near-stall |
| Future goal: 2.5 m/s + 20° ramp | out of scope v0.1 | 3.45 Nm @ 398 RPM | see §5 |

Reproduce the straight-line cases:

```bash
python3 robot_ws/tools/torque_sweep.py --mass-kg 20 --wheel-radius-m 0.06 \
...
gives **2.9 Nm per wheel** — pivot turns, not straight-line driving, size the
stall requirement for a 20 kg skid-steer robot.

### Selection targets per motor

- **Loaded speed ≥ 120 RPM** (no-load ≥ ~160 RPM at 12 V)
- **Rated/continuous torque ≥ 0.75 Nm** (covers ramp+accel without SF; SF-2.0 target 1.5 Nm short-duty)
- **Stall torque ≥ 3 Nm (~30 kg·cm)** so carpet pivots don't stall the drivetrain
...
| **Pololu 37D 70:1 w/ 64 CPR encoder** ([specs](https://www.pololu.com/product/4754/specs)) | $244 ($60.95 ea) | 150 RPM | 2.65 Nm (27 kg·cm), 5.5 A | 0.98 Nm continuous (10 kg·cm) | 4480 CPR at output | yes | Good quality, documented ratings, but stall < pivot requirement — carpet pivots marginal; slower and pricier than goBILDA. |
...
Sizing for the future goals (2.5 m/s, 20° ramp) simultaneously requires ~398 RPM
**and** 3.45 Nm per wheel — roughly a 140 W-class motor per corner, incompatible
with the $300–600 budget and 0.30 m footprint. v0.1 sizes for ≤ 1.0 m/s and 5°
ramps; the goBILDA cartridge swap (26.9:1 → 13.7:1, 435 RPM) is the future speed
path *if* mass comes down or ramps stay shallow. This supersedes nothing — it
implements conflict #1/#3 in `design_conflicts.md`.
