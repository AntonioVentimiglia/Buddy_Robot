...
buddy_ros2_robot/
  ...
  docs/system_model/              How it fits together — start at system_integration.md
  ...
  electronics/                    Generated KiCAD schematics (+ PDFs) and datasheets
  devops/                         Jetson setup, containers, systemd, udev, OTA, networking
  tools/                          Build, figures, KiCAD generators, checkers, site generator
  assets/figures/                 Generated figures — every one produced by tools/figures/
```

No diagram in this repository is drawn by hand. Figures are generated by scripts
in `tools/figures/`, schematics by `tools/kicad/`, and both are regenerated by
`python3 tools/build.py`. A hand-made architecture diagram silently contradicted
an ADR for two weeks before it was caught and deleted; see milestone M03.
