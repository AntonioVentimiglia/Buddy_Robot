...
`tools/kicad/check_drive_wiring.py`).

**This file is the source; the others are checked against it.** The tables below
are *parsed* by `tools/pin_map.py` and used to draw
[`assets/figures/interconnect.svg`](../../../assets/figures/interconnect.svg), and
`tools/check_integration_map.py` fails the build if they ever disagree with
`pins.h` or with the schematic's net table. Edit here first, then rebuild —
`python3 tools/build.py` will tell you what no longer matches.
